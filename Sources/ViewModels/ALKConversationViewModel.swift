...
//            }
            ...
            if messageModel.isMyMessage {
                ...
                height  = ALKPaymentCell.rowHeigh(viewModel: messageModel, width: maxWidth)
    ...
    open func updateMessageMetaData(message: ALMessage) {

        //        if(message.metadata != nil){
        //            var nsmutableMeatdata = NSMutableDictionary();
        //            nsmutableMeatdata = (message.metadata)!
        //            if( nsmutableMeatdata["hiddenStatus"] != nil &&  nsmutableMeatdata["hiddenStatus"] as! String  == "true"  ){
        //                let messageToDelete = alMessages.filter { ($0 as? ALMessage) == message }
        //                guard let messageDel = messageToDelete.first,
        //                    let index = (alMessages as? [ALMessage])?.index(of: messageDel) else {
        //                        return
        //                }
